...
class SI_Widget_Info extends WP_Widget {
    public function __construct(){
        parent::__construct('si_widget_info', 'SportIsland - Информация', [
            ...
            'description' => 'Виджет выводит информацию о спорт-клубе. Основное использование на странице контактов, под картой'
        ]);
    }
...
<p>
    <label for="<?php echo $this->get_field_id('id-option'); ?>">
        Выберите вариант отображения:
    </label>
    <select 
        id="<?php echo $this->get_field_id('id-option'); ?>" 
    ...
    >
        ...
        <?php selected($instance['option'], $option, true); ?>
        >
        <?php echo $desc; ?>
        </option>
        <?php endforeach; ?>
